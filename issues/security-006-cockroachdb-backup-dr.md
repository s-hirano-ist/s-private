...
CockroachDB Cloud Basic は **24時間間隔・30日保持** の管理バックアップ（スケジュール変更不可）を提供する。リストアは Cloud Console から実行できる。Supabase 時代に明示的なバックアップ運用が無かったため、移行を機にリストア手順を把握し、データ保護を確実にする。

## タスク

- [ ] Cloud Console で管理バックアップ（24h 間隔 / 30日保持）が有効なことを確認
- [ ] **リストアのドライラン**: 別クラスタ or 別 DB へ復元してみて手順・所要時間を把握
- [ ] **Supabase 解約前の最終確認**: ETL の count 一致 + 主要レコードのサンプル目視で移行完全性を確認してから解約
