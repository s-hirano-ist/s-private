...
function DevProfilerWrapper({
	...
	const onRender: ProfilerOnRenderCallback = (
		...
		if (actualDuration > threshold) {
			// oxlint-disable-next-line no-console -- this development-only component reports slow renders.
			console.warn(
				`[Profiler] ${renderedId} ${phase}: ${actualDuration.toFixed(1)}ms (threshold: ${threshold}ms)`,
			);
		} else {
			// oxlint-disable-next-line no-console -- this development-only component reports render timing.
			console.debug(
