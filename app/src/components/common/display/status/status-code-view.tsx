...
import {
	...
	EmptyStateIcon,
	EmptyStateTitle,
} from "@s-hirano-ist/s-ui/empty-state";
import {
	AlertTriangleIcon,
	ClockIcon,
	InboxIcon,
...
export function StatusCodeView({
	...
	return (
		<EmptyState data-testid="status-code-view">
			<EmptyStateIcon>{STATUS_ICON_MAP[statusCode]}</EmptyStateIcon>
			<EmptyStateTitle className="bg-linear-to-r from-primary to-primary-grad bg-clip-text text-8xl font-black tracking-tighter text-transparent">
				{statusCode}
			</EmptyStateTitle>
			<EmptyStateDescription>{statusCodeString}</EmptyStateDescription>
		</EmptyState>
	);
}
