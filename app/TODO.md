...
#### 新規11: ドメインロジックの漏洩 
- **現状**: キャッシュタグ生成（`buildContentCacheTag`）がアプリケーションサービス層にある
- **問題**: インフラ関心事（キャッシュ）がアプリケーション層に漏洩、権限チェックも混在
- **改善案**: キャッシュタグ生成をインフラ層へ、権限チェックをドメインサービスへ分離
- **影響度**: 🟡 **中** - アーキテクチャ整合性
- **具体例**: `src/application-services/*/add-*.ts`でのキャッシュタグ構築、権限チェック処理
...
### 新規A: スクリプトでのドメインサービス直接インスタンス化
- **現状**: `scripts/update-and-create.ts`でもドメインサービスを直接インスタンス化
- **問題**: アプリケーションサービスと同じ依存性注入の問題がスクリプトにも存在
- **影響度**: 🟠 **低** - スクリプトは開発時のみ使用、本番影響なし

### 新規B: 権限チェックとビジネスロジックの分離不足  
- **現状**: `hasDumperPostPermission()`がアプリケーションサービス内で直接実行
- **問題**: 認証・認可ロジックがビジネスロジックと混在
- **改善案**: 権限チェックをミドルウェア層またはドメインサービスに分離
- **影響度**: 🟡 **中** - セキュリティとアーキテクチャの分離

### 新規C: キャッシュタグ生成ロジックの配置問題
- **現状**: `buildContentCacheTag`がcommon/utilsにあるが、アプリケーション層から呼び出し
- **問題**: インフラストラクチャ関心事（キャッシュ）の責任範囲不明確
- **改善案**: キャッシュタグ生成をインフラストラクチャ層に移動、またはドメインサービス経由で抽象化
- **影響度**: 🟡 **中** - アーキテクチャ整合性
