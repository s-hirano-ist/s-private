...
├── {action}.ts         ← Server Action wrapper（認証のみ）
...
| `*.ts` | Server Action（認証チェック後にCoreを呼び出し） | あり |
