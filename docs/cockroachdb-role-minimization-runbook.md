...
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE
  ...
  "prod-db".public."user", "prod-db".public."session",
  "prod-db".public."account", "prod-db".public."verification"
TO "s-prod-runtime";
```
`"user"` 等の Better Auth テーブルは Status enum・シーケンスを使わない（id は STRING、`created_at` は CURRENT_TIMESTAMP）ため追加の TYPE/シーケンス権限は不要。`user` は予約語なのでダブルクォート必須。

**再発防止（重要）:** 上記の列挙 GRANT は「その時点で存在するテーブル」にしか効かず、`migrations/**` で新テーブルを追加するたびに手動 GRANT を足さないと runtime が `42501 (insufficient_privilege)` で落ちる（Better Auth 移行時に `verification` への INSERT で実際に発生）。以後 `s-prod`（owner/migrator）が作るテーブルへ自動で DML を付与しておく:

```sql
ALTER DEFAULT PRIVILEGES FOR ROLE "s-prod" IN SCHEMA "prod-db".public
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO "s-prod-runtime";
```
DEFAULT PRIVILEGES は「以後に作られる」オブジェクトにのみ効くため、既存テーブルへの列挙 GRANT は別途必要（＝上のブロック）。これ以降に `prisma:deploy` で増えるテーブルは手動 GRANT 不要。

**確認:** `SHOW GRANTS FOR "s-prod-runtime";` と `SHOW DEFAULT PRIVILEGES FOR ROLE "s-prod";`
**期待結果:** 上記9テーブルの DML + schema・`Status` USAGE + CONNECT、加えて `s-prod` の default privileges に runtime への TABLES DML が出る。`_prisma_migrations` も DDL も無い。`SHOW GRANTS ON DATABASE "dev-db" FOR "s-prod-runtime";` は **空**（prod→dev 隔離）。
...
| runtime は DML のみ | `SHOW GRANTS FOR "s-prod-runtime";` ／ `SHOW DEFAULT PRIVILEGES FOR ROLE "s-prod";` | 9テーブル DML + USAGE + CONNECT のみ。以後の新テーブルは default privileges で自動付与 |
...
| runtime の default privileges | `ALTER DEFAULT PRIVILEGES FOR ROLE "s-prod" IN SCHEMA "prod-db".public REVOKE SELECT, INSERT, UPDATE, DELETE ON TABLES FROM "s-prod-runtime";`（既存テーブルの GRANT は別途 `REVOKE ... ON TABLE ...`） |
