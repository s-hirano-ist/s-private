...
CockroachDB Cloud Basic は接続プーリングが内蔵で、Supabase の Supavisor のような pooled / direct の二重エンドポイントを持ちません。よってアプリ実行も `prisma migrate` も `DATABASE_URL` 1本で足ります（Supabase 時代の `DIRECT_URL` は撤去済み）:
