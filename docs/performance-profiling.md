...
| `enabled` | `boolean` | `true` | `false` の場合は計測せず `children` をそのまま返す。本番で無効化に使う |
...
`enabled` prop が `false` の場合、`children` をそのまま返しオーバーヘッドゼロ（デフォルトは `true`）。本番では呼び出し側で `enabled={process.env.NODE_ENV === "development"}` のように渡して無効化する（`BaseCardStackWrapper` での実例を参照）。
...
`app/next.config.mjs` で `reactCompiler` を一時的に無効化して比較可能：
