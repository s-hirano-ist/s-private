...
describe("booksEntity", () => {
	...
	describe("bookEntity.markAsLastUpdated", () => {
		test("should transition book from UNEXPORTED to LAST_UPDATED", () => {
			const book = bookEntity.create({
				userId: makeUserId("test-user-id"),
				ISBN: makeISBN("9784123456789"),
				title: makeBookTitle("Test Book"),
			...
			const lastUpdatedBook = bookEntity.markAsLastUpdated(book);

			expect(lastUpdatedBook.status).toBe("LAST_UPDATED");
			expect(lastUpdatedBook.id).toBe(book.id);
			expect(lastUpdatedBook.ISBN).toBe(book.ISBN);
		});
		...
		test("should return frozen object", () => {
			...
			expect(Object.isFrozen(lastUpdatedBook)).toBe(true);
		});
	});

	describe("bookEntity.revert", () => {
		test("should transition book from LAST_UPDATED back to UNEXPORTED", () => {
			const book = bookEntity.create({
				userId: makeUserId("test-user-id"),
				ISBN: makeISBN("9784123456789"),
				title: makeBookTitle("Test Book"),
			...
			const revertedBook = bookEntity.revert(lastUpdatedBook);

			expect(revertedBook.status).toBe("UNEXPORTED");
			expect(revertedBook.id).toBe(book.id);
			expect(revertedBook.ISBN).toBe(book.ISBN);
		});
		...
		test("should return frozen object", () => {
			...
			expect(Object.isFrozen(revertedBook)).toBe(true);
		});
	});

	describe("bookEntity.finalize", () => {
		test("should transition book from LAST_UPDATED to EXPORTED", () => {
			const book = bookEntity.create({
				userId: makeUserId("test-user-id"),
				ISBN: makeISBN("9784123456789"),
				title: makeBookTitle("Test Book"),
			...
			const exportedBook = bookEntity.finalize(lastUpdatedBook);

			expect(exportedBook.status).toBe("EXPORTED");
			expect(exportedBook.exportedAt).toBeInstanceOf(Date);
			expect(exportedBook.id).toBe(book.id);
			expect(exportedBook.ISBN).toBe(book.ISBN);
		});
		...
		test("should return frozen object", () => {
			...
			expect(Object.isFrozen(exportedBook)).toBe(true);
		});
	});
});
