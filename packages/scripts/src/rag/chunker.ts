...
export async function testChunking(): Promise<void> {
	...
	const jsonChunks = parseJsonArticle("test.json", testJson);
	console.log("JSON chunks:", JSON.stringify(jsonChunks, null, 2));
...
`;
	...
	const mdChunks = parseMarkdown("test.md", testMd);
	console.log("Markdown chunks:", JSON.stringify(mdChunks, null, 2));
}
